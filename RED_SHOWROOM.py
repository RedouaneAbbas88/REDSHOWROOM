...
def selection_produit(df):
    ...
    familles_dispo = df[(df['Marque'] == marque_choisie) & (df['Catégorie'] == categorie_choisie)][
        'Famille'].dropna().unique().tolist()
    famille_choisie = st.selectbox("Famille *", familles_dispo)

    produits_dispo = \
    df[(df['Marque'] == marque_choisie) & (df['Catégorie'] == categorie_choisie) & (df['Famille'] == famille_choisie)][
            ...
            if vente_valide:
                ...
                for item in st.session_state.panier:
                    row_vente = [
                        ...
                        round(item["Total"] * 1.19, 2),  # Total TTC
