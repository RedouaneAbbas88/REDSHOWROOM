...
def selection_produit(df, prefix=""):
    ...
    if marque_key not in st.session_state or st.session_state[marque_key] not in marques_dispo:
    ...
    categorie_choisie = st.selectbox("Catégorie *", categories_dispo, index=categories_dispo.index(st.session_state[categorie_key]) if categories_dispo else 0)
    ...
    famille_choisie = st.selectbox("Famille *", familles_dispo, index=familles_dispo.index(st.session_state[famille_key]) if familles_dispo else 0)

    # Produit
    produits_dispo = df[(df['Marque'] == marque_choisie) &
                        (df['Catégorie'] == categorie_choisie) &
                        (df['Famille'] == famille_choisie)]['Produit'].dropna().tolist()
    ...
    produit_choisi = st.selectbox("Produit *", produits_dispo, index=produits_dispo.index(st.session_state[produit_key]) if produits_dispo else 0)
...
# -----------------------------
    ...
    with st.form("form_vente"):
        ...
        montant_paye = st.number_input("Montant payé", min_value=0.0, max_value=total_vente*1.19, step=1.0)
    ...
    if st.session_state.panier:
        ...
        for i, item in enumerate(st.session_state.panier):
            ...
            with col2:
                ...
                st.session_state.panier[i]["Total"] = nouvelle_qte * item["Prix unitaire"]
            with col3:
